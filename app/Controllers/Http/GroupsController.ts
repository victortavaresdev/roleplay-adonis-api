...
export default class GroupsController {
  public async index({ request, response }: HttpContextContract) {
    ...
    // let groups = await this.filterByQueryString(text, userId)

    const groupsQuery = await this.filterByQueryString(text, userId)
  ...
  private async filterByQueryString(userId: number, text: string) {
  ...
  private async filterByUser(userId: number) {
    return Group.query()
      .preload('players')
      .preload('masterUser')
  ...
  private async filterByText(text: string) {
    return Group.query()
      .preload('players')
      .preload('masterUser')
  ...
  private async filterByUserAndText(userId: number, text: string) {
    return Group.query()
      .preload('players')
      .preload('masterUser')
      .withScopes((scope) => scope.withPlayer(userId))
