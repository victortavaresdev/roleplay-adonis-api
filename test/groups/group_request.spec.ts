...
test.group('Group Request', (group) => {
  test('it should create an group request', async (assert) => {
  ...
  test('it should return 404 when proving an unexisting group', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
  ...
  test('it should return 404 when proving an unexisting group request', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
